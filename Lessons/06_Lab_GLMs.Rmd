...
```{r}
#Exercise 1:
Density.Ozone<- ggplot(EPAair, aes(x= Ozone, fill= as.factor(Year))) + 
  geom_density(alpha = 0.5) + 
  scale_fill_manual(values=c("darkgreen", "darkblue")) +
  labs(x= "Ozone AQI Value", y= "Density", fil = "")
print(Density.Ozone)
...
EPAair.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(EPAair.regression)
...
EPAair.Corr<- cor(EPAair.Omit)
corrplot(EPAair.Corr, method = "ellipse")
corrplot.mixed(EPAair.Corr, upper = "ellipse")
view(EPAair.Corr)

#greatest positive correlations between Ozone and PM2.5 and PM2.5 and meanLat.
...
EPAAIC<- lm(data= EPAair.Omit, PM2.5 ~ meanLat + meanLong + Month + Year + Ozone)
summary(EPAAIC)
step(EPAAIC)
...
OzoneBest.Regression <- lm(data = EPAair.Omit, PM2.5~ meanLat + meanLong + Month + Ozone)
summary(EPAAIC)
```
...
Litter.Totals.groups$groups
 #add column with grouping from above
Litter.Totals <- Litter %>%
...
assign.group <- function(site)
  return(Litter.Totals.groups$groups[as.character9site02])
