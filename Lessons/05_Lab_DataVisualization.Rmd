...
temp.plot1 <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), aes(x=daynum, y = temperature_C)) + geom_point(aes(color = year4)) + facet_wrap(vars(lakename), nrow= 2) + ylab(expression("Surface Temp"))
print(temp.plot1)
...
temp.plot2<- ggplot(PeterPaul.chem.nutrients, aes(x= sampledate, y= temperature_C)) + geom_point(aes(color= depth), size=0.5) + ylab("Temperature") + scale_color_viridis(option ="magma", direction=1) +scale_x_date(date_breaks= "5 years", date_labels = "%Y")
print(temp.plot2)
...
plot.airquality <- ggplot(EPAair, aes(x=PM2.5, y= Ozone)) + geom_point(aes(color= meanLat), alpha=0.5) + geom_smooth(method=lm, se=FALSE)
print(plot.airquality)
...
# Choose which plot displays the data best and justify your choice. 
  ggplot(EPAair, aes(x=Month,y=PM2.5, color=Year))+ geom_point() + scale_x_continuous(limits=c(1,12), breaks =c(1:12))

ggplot(EPAair, aes(x=as.Factor(Year),y=PM2.5)) + geom_point()

ggplot(EPAair, aes(x=Month,y=PM2.5))+ geom_point() + facet_wrap(vars(Year), nrow=2) +scale_x_continuous(limits=c(1,12), breaks=c(1:12))

ggplot(EPAair,aes(x=as.factor(Year), y=PM2.5))+ geom_violin()
