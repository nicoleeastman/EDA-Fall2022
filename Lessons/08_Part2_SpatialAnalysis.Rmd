...
#Have a look the variety of types (and number of each), counts types
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
...
huc_8_wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
...
huc_sf_wind <-huc8_sf %>%
  left_join(huc_8_wind, by = c('HUC_8', 'HUC_8'))

#or
huc_8_sf_wind <- merge(x= huc8_sf, y= huc_8_wind, by.x = "HUC_8", by.y = "HUC_8")

#View the outputs
mapview(huc_8_sf_wind, zcol = "kWhkm2day")
ggplot(data=huc_8_sf_wind) +
...
the_triange<- counties_sf_utm %>%
  filter(NAME %in% c("Chatham", "Durham", "Wake", "Orange", "Chatham"))

#Grab intersecting HUC-8s
triangle_HUCs <- huc8_sf_utm %>%
  filter(st_intersects(x=., y= triangle_co, sparse = F))
triangle_HUCs <- huc8_sf_utm[the_triange,]

ggplot() +
  geom_sf(data= the_triange, color='grey') +
  geom_sf(data= huc8_sf_utm, color='green', fille= NA) +
