...
#Set the URL, can easily edit the url
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=7&format=rdb'

#Get the data, which starts on line 30, use read.table for tab-separated data, skip= 29 is to skip first 29 lines before we reach the data
...
URL.Eno <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

eno_gage_data <- read.table(URL.Eno,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 

colnames(eno_gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

eno_gage_data <- eno_gage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

ggplot(eno_gage_data,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
df_values <- data.frame("State" = as.character(the_states),
                               "Avg_retail_price" = the_price,
                               "Net_summer_capcity" = the_capacity,
                                "Net_generation" = net_generation,
                                "Total_retail_sales" = total_retail)
...
#1 Link to the web site using read_html
the_website <- read_html(paste0(the_url))
...
df_many.years<- map(seq(2017,2019),scrape.it) %>% bind_rows()
