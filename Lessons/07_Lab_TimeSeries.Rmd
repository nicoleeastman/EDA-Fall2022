...
Rainfall.Trend <- ggplot(BRA_complete, aes(x=Date, y=Rainfall)) +
  geom_line() +
  geom_smooth()

Temp.Trend<-ggplot(BRA_complete, aes(x=Date, y=Temperature_C)) +
  geom_line() +
  geom_smooth()
...
BRA.rainfall.ts<- ts(BRA_complete$Rainfall, start= c(1901,1), frequency= 12)
head(BRA.ranfall.ts)

BRA.temperature.ts<- ts(BRA_complete$Temperature_C, frequency= 12, start= c(1901,1))
head(BRA.temperature.ts)
...
BRA.rainfall_Decomposed <- stl(BRA.rainfall.ts, s.window = "periodic")
BRA.temperature_Decomposed <- stl(BRA.temperature.ts, s.window = "periodic")

plot(BRA.rainfall_Decomposed)
plot(BRA.temperature_Decomposed)
...
BRA.Rainfall.Components <- as.data.frame(BRA.rainfall_Decomposed$time.series[,1:3])

BRA.Temp.Components <- as.data.frame(BRA.temperature_Decomposed$time.series[,1:3])

BRA.Rainfall.Components <- mutate(BRA.Rainfall.Components, Observed = BRA_complete$Rainfall,
                                 Date= BRA_complete$Date)
BRA.Temp.Components <- mutate(BRA.Rainfall.Components, Observed = BRA_complete$Rainfall,
                                 Date= BRA_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(BRA.Rainfall.Components) +
  geom_line(aes(y= Observed, x= Date) , size = 0.25) +
  geom_line(aes(y= trend, x= Date), color= "blue") +
  ylab("Rainfall")

ggplot(BRA.Temp.Components) +
  geom_line(aes(y= Observed, x= Date) , size = 0.25) +
