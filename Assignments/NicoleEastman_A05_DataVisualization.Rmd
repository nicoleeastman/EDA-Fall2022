...
Box.Temp <- 
  ggplot(PP.Nutrients, aes(x= (month.abb[month]), y= temperature_C, color= lakename)) +
           geom_boxplot() +
           scale_x_discrete(drop=FALSE) +
  labs(x="Month", y = "Temperature (C)") +
  scale_x_date(limits= as.Date(c("1", "12")))
print(Box.Temp)

#5b
Box.TP <- ggplot(PP.Nutrients, aes(x= (month.abb[month]), y= tp_ug)) +
...
#5c
Box.TN <- ggplot(PP.Nutrients, aes(x= (month.abb[month]), y= tn_ug)) +
  geom_boxplot(aes(color= lakename)) +
...
#5d Cowplot
Nutrients.Cowplot <- 
  plot_grid(Box.Temp, Box.TP, Box.TN, labels= c('Temp', 'TP', 'TN'), common.legend= TRUE)
...
Needles.Plot <-
  ggplot(filter(Niwot.Litter, functionalGroup == "Needles"), aes(x= collectDate, y= dryMass, color= nlcdClass)) +
           geom_point() + 
  labs(x= "Date", y= "Mass") +
scale_fill_discrete(name= "NLCD Class", labels= "Evergreen Forest", "Grassland Herbaceuous", "Shrub Scrub" ) +
