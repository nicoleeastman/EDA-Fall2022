...
# 5
Days<- as.data.frame(GaringerOzone.Processed$Date, seq(2010-01-01:2019-12-31))
...
```{r}
#9
GaringerOzone.monthly <- Ozone.Clean %>%
  separate(Ozone.Clean,c("Null","Month","Null2")," ") %>% 
...
```{r}
#10
GaringeOzone.daily.ts <- ts(Ozone.Clean$Ozone.Clean, start = c(2010,1), frequency= 365)

GaringerOzone.monthly.ts <- ts(GraingerOzone.monthly$Ozone.Clean, start = c(,1), frequency = 12)
...
```{r}
# 13
Ozone.MK.Plot <- ggplot(GaringerOzone.monthly, aes(x= Date, y= Ozone.Clean)) +
  geom_point() +
  geom_line() +
  xlab("Month") +
...
#16
Ozone.Extracted.ts <- ts(Ozone.Seasonal.Components$Observed, start = c(,1), frequency = 12)
